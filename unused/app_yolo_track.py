...
object_timestamps = {}  # Stores the last update time of each object
...
bounding_boxes = (800, 100, 1000, 350)

# Load YOLO model (ONNX format)
model = YOLO('./models/yolov5su.onnx')
...
def is_inside_bbox(x1, y1, x2, y2, bbox):
    ...
    inside = bx1 <= center_x <= bx2 and by1 <= center_y <= by2

    # print(f"📍 Object {center_x:.1f},{center_y:.1f} inside? {inside}")
    return inside
...
def generate_frames():
    ...
    while True:
        ...
        # 🔥 Use YOLO tracking instead of manual tracking
        ...
        for det in detections:
            x1, y1, x2, y2, conf, cls, track_id = det[:7]  # YOLO tracking assigns unique IDs
            
            if int(cls) != 0:  # Filter for "person" class
                continue

            obj_id = int(track_id) if not np.isnan(track_id) else assign_object_id(x1, y1, x2, y2)
            inside = is_inside_bbox(x1, y1, x2, y2, bounding_boxes)

            if obj_id not in object_status:
                object_status[obj_id] = "outside"
                object_timestamps[obj_id] = current_time  # 🔥 Initialize timestamp

            prev_status = object_status[obj_id]  
            time_since_last_change = current_time - object_timestamps[obj_id]

            # 🔥 Prevent immediate flip-flopping with cooldown
            if prev_status == "outside" and inside and (current_time - object_timestamps[obj_id]) > cooldown_time:
                print(f"🔵 Person {obj_id} ENTERED the bounding box!")
                object_status[obj_id] = "inside"
                object_timestamps[obj_id] = current_time  # Update timestamp
            elif prev_status == "inside" and not inside and (current_time - object_timestamps[obj_id]) > cooldown_time:
                object_status[obj_id] = "outside"
